...
// 관리자 암호를 단방향 암호화(해시알고리즘) 하기위해 bcryptjs 패키지 참조하기
...
router.post("/modify", async (req, res, next) => {
  ...
  // db 서버에 해당 관리자계정 정보를 수정하고 실제 수정된 건수를 db서버에 반환한다.
  // update() => UPDATE admin SET company_code = 0, ..... WHERE admin_member_id = 1;
  const updateCnt = await db.Admin.update(admin, {
...
router.get("/delete", async (req, res, next) => {
  ...
  // Step2: 관리자 교유번호에 해당하는 단일 관리자 정보를 삭제처리한다.
  const deletedCnt = await db.Admin.destroy({
    where: { admin_member_id: admin_member_id },
  });

  // Step3: 삭제 후 목록 페이지로 이동한다.
  res.redirect("/admin/list");
});
